...
namespace DemoTesting
{
    ...
    public class TClientPresenter
    {
        ...
        public void TestConstruct()
        {
            ...
            var viewObjects = mockViews.Select(v => v.Object).ToList();
            var clientPresenter = new ClientPresenter(mockModel.Object, viewObjects);

            // ВЫЗОВ МЕТОДА, КОТОРЫЙ ДОЛЖЕН ВЗАИМОДЕЙСТВОВАТЬ С ПРЕДСТАВЛЕНИЯМИ
            // Например, если у презентера есть метод LoadClients или аналогичный:
            clientPresenter.LoadClients(); // или другой метод, который вызывает ShowClientInfo

            // ПРОВЕРКА С ПОМОЩЬЮ VERIFY
            for (int i = 0; i < countClients; i++)
            {
                Client expectedClient = testClients[i];
                mockViews[i].Verify(v => v.ShowClientInfo(expectedClient), Times.Once);
            }
    }
}
